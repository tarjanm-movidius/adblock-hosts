...
if [ -n "$DNSSRV" ]; then
	HOSTS="dns_hosts-${DNSSRV}"
	[ ! -e "$HOSTS" ] && scp -O $DNSSRV:/etc/hosts "./$HOSTS"
	if [ ! -e "$DNSLOG" ]; then
...
[ -n "$DEVSHMTMP" ] && cp "$HOSTS" "${DEVSHMTMP}hosts" && HOSTS="${DEVSHMTMP}hosts"
...
grep "ads\|\bad\|analytics" "$NONBLOCKED_SERVERS" | grep -v "uploads\|downloads\|addon\|iadsdk.*apple.com\|in-addr\.arpa\|adjust\.com\|\.profile.*cloudfront\.net\|vod-adaptive\.akamaized\.net\|adl\.windows\.com\|mcafee.com"
